...
const Dashboard = (props) => {
	...
	useEffect(() => {
		fetch('https://suck-your-d-server.herokuapp.com/orders?email=' + auth.user.email)
			.then((res) => res.json())
							...
							<Row className="text-white py-2 px-4">
								<Col md={9}>
									...
									<div className="progress-container">
										<span className="progress-badge progress-text">{item.status}</span>
										<Progress max="100" value={`${item.progress}`}>
											<span className="progress-value progress-text">{item.progress}%</span>
										</Progress>
									</div>
								</Col>
						...
						<div className="jumbotron card card-image background-Order">
							<Row className="text-white px-4 d-flex align-items-center">
								...
								<Col md={3} className="ml-0 mt-1">
									<h4 className="mt-1">{new Date(`${item.shipment.getDate}`).toDateString()}</h4>
								</Col>
